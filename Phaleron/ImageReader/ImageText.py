...
def decode_predictions(scores, geometry):
    ...
    for y in range(0, numRows):
        ...
        for x in range(0, numCols):
            ...
            # use the geometry volume to drive the width and height
            # of the bounding box
            h = xData0[x] + xData2[x]
            w = xData1[x] + xData3[x]
            ...
            # for the text prediction boundng box
...
# load the pre-traied EAST text detector
print('[INFO] loading EAST text detector...')
net = cv2.dnn.readNet(args['east'])

# construct a blob from the image and then perform a forward pass of
# the model to ontain the two output layer sets
...
for (startX, startY, endX, endY) in boxes:
    ...
    # extract the actuall padded ROI
    roi = orig[startY:endY, startX:endX]

    # in order to apply Tessract v4 to OCR text we must supply
    # (1) a language, (2) an OEM flag of 4, indicating that we
...
for ((startX, startY, endX, endY), text) in results:
    ...
    cv2.imshow('Text Detexction', output)
    cv2.waitKey(0)
