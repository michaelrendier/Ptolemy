...
class Pycrypt():#Pycrypt general functions class
	...
	def __init__(self):
		...
		self.frequency_english = 'AAAAAAAABBCCCDDDDEEEEEEEEEEEEEFFGGHHHHHHIIIIIIIJKLLLLMMNNNNNNNOOOOOOOOPPQRRRRRRSSSSSSTTTTTTTTTUUUVWWXYYZ'#English Letter Frequency
	...
	def collectstats(self):#Collectstats function
		...
		if len(Pycrypt.working_text) != 0:
			stats_text_box.insert(END, 'Number of Characters: ' + str(len(Pycrypt.working_text.replace(' ', ''))))
		...
		if len(self.words) != 0:
			stats_text_box.insert(END, 'Number of Words: ' + str(len(self.words)))
			stats_text_box.insert(END, '\n' + '-'*80 + '\n')
	
		#Count Number of Lines of 80 characters
		self.lines = (len(Pycrypt.working_text)/80) + Pycrypt.working_text.count('\n')
		stats_text_box.insert(END, 'Number of Lines X 80: ' + str(self.lines))
		stats_text_box.insert(END, '\n' + '-'*80 + '\n')
		...
		if len(Pycrypt.working_text.split(None)) > 1:
			...
			stats_text_box.insert(END, 'Number of Sentences: ' + str(self.sentence))
			stats_text_box.insert(END, '\n' + '-'*80 + '\n')
		...
		#Create Pycrypt.working_text Character Dictionary
		self.dic1 = {}
		...
		for k in sorted(self.dic1):
			stats_text_box.insert(END, '%s - %d' % (k, self.dic1[k]) + '\n')
		...
		for k in sorted(self.dic1.keys(), key=(self.dic1.get), reverse=True):
			...
			stats_text_box.insert(END, '%s %s %d' % (k, '|' * (int(self.prct)), (self.prct)) + '\n')
		...
		for k in sorted(self.dic2):
			stats_text_box.insert(END, '%s %s %d' % (k, '|'*(self.dic2[k]), self.dic2[k]) + '\n')
...
class Toolbar(Frame):#Toolbar class
	...
	def __init__(self, master):#Create toolbar/buttons
		...
		self.railfence = Button(self.trans, state=DISABLED, image=self.railimg, width=25, height=25, command=self.passnull)##
	...
	def startwork(self):#Power On function
		...
		Pycrypt.original_text = working_text_box.get(1.0, END)
		Pycrypt.working_text = working_text_box.get(1.0, END)
		Pycrypt.change_text = Pycrypt.working_text

		#Change Main Text
		working_text_box.delete(1.0, END)
		working_text_box.insert(END, Pycrypt.working_text)

		#Change change text
		change_text_box.delete(1.0, END)
		change_text_box.insert(END, Pycrypt.change_text)
	...
	def stopwork(self):#Power Off function
		...
		if tkMessageBox.askokcancel('Revert To Original Text?', 'You will loose any changes...'):
			change_text_box.delete(1.0, END)
			working_text_box.delete(1.0, END)
			working_text_box.insert(END, Pycrypt.original_text)
	...
	def updatetext(self):#Create update text function
		...
		working_text_box.delete(1.0, END)
		working_text_box.insert(1.0, Pycrypt.working_text)
		change_text_box.delete(1.0, END)
		change_text_box.insert(1.0, Pycrypt.working_text)
		Pycrypt.change_text = ''
...
class Replace(Toplevel):#Replace Options class
	...
	def cancel(self, event=None):#Replace Optios cancel button/window protocol
		...
		Pycrypt.change_text = Pycrypt.working_text
		change_text_box.delete(1.0, END)
		change_text_box.insert(END, Pycrypt.working_text)
...
class Scytale(Toplevel):#Scytale Shift class
	...
	def scytaleshift(self):#Scytale Shift function
		...
		Pycrypt.change_text = Pycrypt.working_text
		self.scytale = []		
		self.matrix = [ [ [] for i in range(15) ] for j in range(1) ]
		...
		Pycrypt.change_text = change_text_box.get(1.0, END)
...
class Scrabble(Toplevel):#Scrabble Cipher class
	...
	def scrabbledone(self, event):#Commit changes button event
		...
		Pycrypt.working_text = change_text_box.get(1.0, END)
		working_text_box.delete(1.0, END)
		working_text_box.insert(END, Pycrypt.working_text)
...
#Instantiate Pycrypt, Topmenu and Toolbar
Pycrypt = Pycrypt()
menu = Topmenu(root)
tools = Toolbar(root)
...
working_text_box = Text(working_frame, yscrollcommand=working_scroll.set, height=17, width=80, insertbackground='white', fg='white', bg='black')
...
change_text_box = Text(change_frame, yscrollcommand=change_scroll.set, height=17, width=80, insertbackground='white', fg='green', bg='black')
...
stats_text_box = Text(stats_frame, yscrollcommand=stats_scroll.set, height=36, width=80, fg='white', bg='black')
