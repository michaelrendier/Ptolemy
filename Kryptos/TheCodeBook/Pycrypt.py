...
class Pycrypt():  # Pycrypt general functions class
	...
	def __init__(self, parent=None):
		...
		self.freq_dict = {}  # Frequecy analysis dictionary
		...
		self.en_dict = {}  # English frequency analysis dictionary
		self.en_letter_freq = []  # List of english percent letter frequency
		self.work_letter_freq = []  # List of working text percent letter frequency
		self.alphaletters = []  # list of alphabetical letters
	...
	def body(self):
		...
		self.working_text_box = Text(working_frame, yscrollcommand=working_scroll.set, height=17, width=80,
									 insertbackground='white', fg='white', bg='black')
		self.working_text_box.grid(row=0, column=0)
		# self.working_text_box.delete(FIRST, END)
		...
		self.change_text_box = Text(change_frame, yscrollcommand=change_scroll.set, height=17, width=80,
									insertbackground='white', fg='green', bg='black')
		...
		self.stats_text_box = Text(stats_frame, yscrollcommand=stats_scroll.set, height=36, width=40, fg='white',
								   bg='black')
	...
	def collectstats(self):  # Collectstats function
		...
		if len(Pycrypt.working_text) != 0:
			Pycrypt.stats_text_box.insert(END,
										  'Number of Characters: ' + str(len(Pycrypt.working_text.replace(' ', ''))))
			Pycrypt.stats_text_box.insert(END, '\n' + '-' * 80 + '\n')
		...
		if len(Pycrypt.working_text.split(None)) > 1:
			self.sentence = Pycrypt.working_text.count('.') + Pycrypt.working_text.count(
				'!') + Pycrypt.working_text.count('?')
		...
		for k in sorted(self.freq_dict):
			Pycrypt.stats_text_box.insert(END, '%s - %d' % (k, self.freq_dict[k]) + '\n')
		...
		for k in sorted(self.freq_dict.keys(), key=self.freq_dict.get, reverse=True):
			Pycrypt.stats_text_box.insert(END, '%s - %d' % (k, self.freq_dict[k]) + '\n')
		...
		self.work_letter_freq = []
		
		for k in sorted(self.freq_dict.keys(), key=(self.freq_dict.get), reverse=True):
			self.prct = ((float(self.freq_dict[k]) / float(len(Pycrypt.working_text))) * 100)
			Pycrypt.stats_text_box.insert(END, '%s %s %d' % (k, '|' * (int(self.prct)), (self.prct)) + '\n')
			self.work_letter_freq.append(str(k.upper()))
		...
		for k in sorted(self.en_dict):
			Pycrypt.stats_text_box.insert(END, '%s %s %d' % (k, '|' * (self.en_dict[k]), self.en_dict[k]) + '\n')
		...
		for k in sorted(self.en_dict.keys(), key=self.en_dict.get, reverse=True):
			Pycrypt.stats_text_box.insert(END, '%s %s %d' % (k, '|' * (self.en_dict[k]), self.en_dict[k]) + '\n')
			self.en_letter_freq.append(k.upper())
...
class Topmenu(Frame):  # Menu Bar class
	...
	def __init__(self, parent=None):  # Create the menu/filemenu
		
		menu = Menu(root)
		root.config(menu=menu)
		self.parent = parent
		print("TOPMENU PARENT:", self.parent)
	...
	def filemenunew(self):  # Create file new function
		self.parent.original_text = ""
		self.parent.working_text = ""
		self.parent.working_text_box.delete(1.0, END)
		pass
	
	def filemenuopen(self):  # Create file open function
		
		self.filename = filedialog.askopenfilename(
			initialdir="/home/rendier/Ptolemy/Kryptos/TheCodeBook",
			title="Select file",
		...
		self.parent.original_text = self.text
		self.parent.working_text = self.text
		self.parent.working_text_box.insert(END, self.text)
		self.file.close()
		pass
	
	def filemenusave(self):  # Create file save function
		
		self.text = self.parent.working_text_box
...
class Toolbar(Frame):  # Toolbar class
	...
	def __init__(self, master):  # Create toolbar/buttons
		...
		self.strip = Button(self.replace, state=DISABLED, image=self.replaceimg, width=25, height=25,
							command=self.raisereplace)
		...
		self.railfence = Button(self.trans, state=DISABLED, image=self.railimg, width=25, height=25,
								command=self.raiserailfence)
		...
		self.scytale = Button(self.trans, state=DISABLED, image=self.scyimg, width=25, height=25,
							  command=self.raisescytale)
		...
		self.caesar = Button(self.substitute, state=DISABLED, image=self.caesarimg, width=25, height=25,
							 command=self.raisecaesar)
		...
		self.pigpen = Button(self.substitute, state=DISABLED, image=self.pigimg, width=25, height=25,
							 command=self.passnull)  ##
		...
		self.scrabble = Button(self.substitute, state=DISABLED, image=self.monoimg, width=25, height=25,
							   command=self.raisescrabble)
		...
		self.playfair = Button(self.advanced, state=DISABLED, image=self.playimg, width=25, height=25,
							   command=self.raiseplayfair)
		...
		self.digraph = Button(self.advanced, state=DISABLED, image=self.digraphimg, width=25, height=25,
							  command=self.passnull)  ##
		...
		self.homophonic = Button(self.advanced, state=DISABLED, image=self.homoimg, width=25, height=25,
								 command=self.passnull)  ##
		...
		self.vigener = Button(self.advanced, state=DISABLED, image=self.vigenerimg, width=25, height=25,
							  command=self.passnull)  ##
		...
		self.enigma = Button(self.advanced, state=DISABLED, image=self.enigmaimg, width=25, height=25,
							 command=self.passnull)  ##
		...
		self.freqlet = Button(self.stats, state=DISABLED, image=self.freqletimg, width=25, height=25,
							  command=self.passnull)  ##
		...
		self.freqdouble = Button(self.stats, state=DISABLED, image=self.freqdouimg, width=25, height=25,
								 command=self.passnull)  ##
		...
		self.voweltrowel = Button(self.stats, state=DISABLED, image=self.vowelimg, width=25, height=25,
								  command=self.passnull)  ##
...
class Railfence(Toplevel):  # Railfence class#TODO#
	...
	def body(self, master):
		...
		self.rail1 = Radiobutton(self.buttonframe, command=self.railfence, text='2', width=3, bd=2, variable=self.rf,
								 value=0, indicatoron=0)
		self.rail1.grid(row=0, column=0, padx=2, pady=2)
		self.rail1.deselect()
		
		self.rail2 = Radiobutton(self.buttonframe, command=self.railfence, text='3', width=3, bd=2, variable=self.rf,
								 value=1, indicatoron=0)
		self.rail2.grid(row=0, column=1, padx=2, pady=2)
		
		self.rail3 = Radiobutton(self.buttonframe, command=self.railfence, text='4', width=3, bd=2, variable=self.rf,
								 value=2, indicatoron=0)
		self.rail3.grid(row=0, column=2, padx=2, pady=2)
		
		self.rail4 = Radiobutton(self.buttonframe, command=self.railfence, text='5', width=3, bd=2, variable=self.rf,
								 value=3, indicatoron=0)
		self.rail4.grid(row=1, column=0, padx=2, pady=2)
		
		self.rail5 = Radiobutton(self.buttonframe, command=self.railfence, text='6', width=3, bd=2, variable=self.rf,
								 value=4, indicatoron=0)
		self.rail5.grid(row=1, column=1, padx=2, pady=2)
		
		self.rail6 = Radiobutton(self.buttonframe, command=self.railfence, text='7', width=3, bd=2, variable=self.rf,
								 value=5, indicatoron=0)
		self.rail6.grid(row=1, column=2, padx=2, pady=2)
		
		self.rail7 = Radiobutton(self.buttonframe, command=self.railfence, text='8', width=3, bd=2, variable=self.rf,
								 value=6, indicatoron=0)
		self.rail7.grid(row=2, column=0, padx=2, pady=2)
		
		self.rail8 = Radiobutton(self.buttonframe, command=self.railfence, text='9', width=3, bd=2, variable=self.rf,
								 value=7, indicatoron=0)
		self.rail8.grid(row=2, column=1, padx=2, pady=2)
		
		self.rail9 = Radiobutton(self.buttonframe, command=self.railfence, text='10', width=3, bd=2, variable=self.rf,
...
class Scytale(Toplevel):  # Scytale Shift class
	...
	def body(self, master):  # Create Scytale Toplevel body
		...
		self.radio2 = Radiobutton(self.frame, text='2', variable=self.pr, width=3, value=0, indicatoron=0,
								  command=self.scytaleshift)
		self.radio2.grid(row=3, column=0)
		self.radio2.deselect()
		
		self.radio3 = Radiobutton(self.frame, text='3', variable=self.pr, width=3, value=1, indicatoron=0,
								  command=self.scytaleshift)
		self.radio3.grid(row=3, column=1)
		
		self.radio4 = Radiobutton(self.frame, text='4', variable=self.pr, width=3, value=2, indicatoron=0,
								  command=self.scytaleshift)
		self.radio4.grid(row=3, column=2)
		
		self.radio5 = Radiobutton(self.frame, text='5', variable=self.pr, width=3, value=3, indicatoron=0,
								  command=self.scytaleshift)
		self.radio5.grid(row=4, column=0)
		
		self.radio6 = Radiobutton(self.frame, text='6', variable=self.pr, width=3, value=4, indicatoron=0,
								  command=self.scytaleshift)
		self.radio6.grid(row=4, column=1)
		
		self.radio7 = Radiobutton(self.frame, text='7', variable=self.pr, width=3, value=5, indicatoron=0,
								  command=self.scytaleshift)
		self.radio7.grid(row=4, column=2)
		
		self.radio8 = Radiobutton(self.frame, text='8', variable=self.pr, width=3, value=6, indicatoron=0,
								  command=self.scytaleshift)
		self.radio8.grid(row=5, column=0)
		
		self.radio9 = Radiobutton(self.frame, text='9', variable=self.pr, width=3, value=7, indicatoron=0,
								  command=self.scytaleshift)
		self.radio9.grid(row=5, column=1)
		
		self.radio10 = Radiobutton(self.frame, text='10', variable=self.pr, width=3, value=8, indicatoron=0,
								   command=self.scytaleshift)
...
class Caesar(Toplevel):  # Caesar Shift class
	...
	def __init__(self, parent):  # Initialize Caesar Toplevel
		...
		self.alphaletters = []  # List of old letters
		self.alphalettersindex = []  # List of old letter indecies
	...
	def body(self, master):  # Create Caesar Shift Toplevel body
		...
		# Create ceaser scale
		self.scale = Scale(self.slider, from_=0, to=25, orient=HORIZONTAL, tickinterval=13, length=210,
						   command=self.caesarshift)
		...
		self.alphalettersters = Label(self.slider, text='ABCDEFGHIJKLMNOPQRSTUVWXYZ', font=Pycrypt.mono_font, fg='blue',
							 width=26)
		self.alphalettersters.bind('<Button-4>', self.scaleforward)
		self.alphalettersters.bind('<Button-5>', self.scaleback)
		self.alphalettersters.grid(row=3, column=0)
		
		# Create caesar shifted 'encoded' letter label
		self.newletters = Label(self.slider, text='ABCDEFGHIJKLMNOPQRSTUVWXYZ', font=Pycrypt.mono_font, fg='red',
								width=26)
	...
	def caesarshift(self, event):  # Caesar Shift function
		...
		self.alphaletters = []
		self.alphalettersindex = []
		self.chgindex = []
		self.chglet = []
		
		# Create initial list of letters
		for c in string.uppercase:
			self.alphaletters.append(c)
		
		# Create list of initial letters indexes
		for c in self.alphaletters:
			self.alphalettersindex.append(self.alphaletters.index(c))
		
		# Encode initial letter indexes
		for c in self.alphalettersindex:
			try:
				self.chgindex.append(self.alphalettersindex.index(c) - self.scale.get())
			
			except ValueError:
				self.chgindex.append(self.alphalettersindex[0])
		
		# Create list of encoded letters
		for c in self.chgindex:
			try:
				self.chglet.append(self.alphaletters[c])
			
			except IndexError:
				self.chglet.append(self.alphaletters[c - 26])
...
class Scrabble(Toplevel):  # Scrabble Cipher class
	...
	def body(self, master):  # Create Scrabble Toplevel body
		...
		self.alphalettersters_frame = LabelFrame(master, text='Scrabble Tiles', bd=2, relief=RIDGE)
		self.alphalettersters_frame.pack()
		
		# Create old scrabble tiles w/itterated radiobuttons
		self.change_this = LabelFrame(self.alphalettersters_frame, text='Change this:', bd=2, relief=RIDGE)
		...
		for i in range(0, 26):
			...
			self.ln1[i] = Radiobutton(self.change_this, state=DISABLED, command=self.ctevent, text=string.uppercase[i],
									  width=1, bd=2, fg='blue', variable=self.ct, value=i, indicatoron=0)
		...
		self.to_this = LabelFrame(self.alphalettersters_frame, text='To this:', bd=2, relief=RIDGE)
		self.to_this.pack()
		...
		for i in range(0, 26):
			self.ln2[i] = Radiobutton(self.to_this, state=DISABLED, command=self.ttevent, text=string.uppercase[i],
									  width=1, bd=2, fg='red', variable=self.tt, value=i, indicatoron=0)
		...
		self.old = Label(self.alphalettersters_frame, text='ABCDEFGHIJKLMNOPQRSTUVWXYZ', font=Pycrypt.mono_font, fg='blue',
						 width=26)
		self.old.pack()
		
		# Create new scrabble list
		self.new = Label(self.alphalettersters_frame, text='', font=Pycrypt.mono_font, fg='red', width=26)
		self.new.pack()
		...
		if len(Pycrypt.work_letter_freq) < 26 or len(Pycrypt.work_letter_freq) > 26:
			self.freq_old.config(text='< 26 Char')
		
		if len(Pycrypt.work_letter_freq) > 26:
			self.freq_old.config(text='> 26 Char')
		
		if len(Pycrypt.work_letter_freq) == 26:
			self.freq_old.config(text=''.join(Pycrypt.work_letter_freq).upper())
		
		# Create scrabble change text frequency list
		self.freq_new = Label(self.analysis, text=''.join(Pycrypt.en_letter_freq), font=Pycrypt.mono_font, fg='red')
	...
	def scrabblefreq(self, event):  # Scrabble Letter Frequency Order event
		...
		for c in Pycrypt.working_text:
			...
			# Replace uppercase with encoded uppercase
			if str.isupper(str(c)):
				i = Pycrypt.work_letter_freq.index(c)
				Pycrypt.change_text = Pycrypt.change_text + str(Pycrypt.en_letter_freq[i])
			
			# Replace lowercase with encoded lowercase
			if str.islower(str(c)):
				i = Pycrypt.work_letter_freq.index(string.upper(c))
				try:
					Pycrypt.change_text = Pycrypt.change_text + string.lower(Pycrypt.en_letter_freq[i])
				except IndexError:
					pass
...
root = Tk()
root.title('Ptolemy - Kryptos - The Code Book')
root.tk.call('wm', 'iconphoto', root._w, PhotoImage(file="/home/rendier/Ptolemy/images/ptol.png"))
